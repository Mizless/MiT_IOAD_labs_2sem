...
class Agent:
    ...
    def _cmd(self, action, target=None):
        ...
        # Throttle: не спамим одну и ту же position-цель
        ...
        # pickup/drop/steal/ready обходят rate-limit — они одноразовые и критичны
    ...
    def _assess_golem_threat(self, pos, golems, eff_vision_dist):
        """
        Возвращает (danger_golem, threat_level, chasing_us):
          0 — нет угрозы
          1 — Patrol видит нас в FOV
          2 — Chase за другим, близко (< 5м)
          3 — Chase за нами
        """
        best_golem  = None
        best_threat = 0
        ...
        for g in golems:
            ...
            if g_state == "Chase":
                ...
                # Patrol — проверяем FOV конус 90° (45° в каждую сторону)
    ...
    def decide(self, me, treasures, bases, game_state, golems, all_agents):
        ...
        # Записываем позицию ВСЕГДА. Старая логика (clear у базы) приводила к тому,
        # что агент мог застрять в коллайдере базы навсегда, так как история очищалась.
        self.pos_hist.append({"x": pos["x"], "z": pos["z"]})
        if len(self.pos_hist) > 25:
            self.pos_hist.pop(0)
            
            # Проверяем разброс координат за последние 25 тиков (2 секунды)
            xs = [p["x"] for p in self.pos_hist]
            zs = [p["z"] for p in self.pos_hist]
            dx = max(xs) - min(xs)
            dz = max(zs) - min(zs)
            
            # Если агент топчется в квадрате 0.5x0.5 метра - он застрял
            if dx < 0.5 and dz < 0.5:
                self.pos_hist.clear()
                
                # Если застряли у базы - отходим ОТ базы (чтобы отлипнуть от коллайдера)
                if near_base:
        ...
        # ================================================================
        # 2. ОЦЕНКА УГРОЗЫ ГОЛЕМОВ
        # ================================================================
        # Если проигрываем, чуть уменьшаем радиус страха перед патрулем
        ...
        # ================================================================
        # 3. РЕАКЦИЯ НА УГРОЗУ ГОЛЕМА (Chase)
        # ================================================================
        ...
        # ================================================================
        # 6. КРАЖА (только при steal_ready=True)
        # ================================================================
        ...
        if steal_ready:
            ...
            for enemy in all_agents:
                ...
                # АДАПТИВНАЯ АГРЕССИЯ:
                # Если проигрываем сильно (>200 очков), приоритет кражи огромный.
                # Если выигрываем (>200 очков), мы не рискуем и воруем только если враг сам подошёл вплотную.
                if score_diff < -200:
                    danger_bonus *= 2.0
                elif score_diff > 200:
                    danger_bonus *= 0.5
            ...
            if best_enemy:
                ...
                # Радиус погони зависит от счёта
                chase_dist = STEAL_CHASE_DIST
                if score_diff < -200:
                    chase_dist = 10.0 # Агрессивно гонимся
                elif score_diff > 200:
                    chase_dist = 3.0  # Гонимся только если враг уже почти в руках
        ...
        if steal_pct < 80:
            ...
            for ally in my_agents_list:
                ...
                for g in golems:
                    if g.get("state") == "Chase" and g.get("targetAgentId") == ally["agentId"]:
            ...
            if escort_needed:
                existing = team_memory["escort_map"].get(escort_needed["agentId"])
                if existing is None or existing == self.id:
                    team_memory["escort_map"][escort_needed["agentId"]] = self.id
        ...
        def base_score(t):
            ...
            # Бонус за близость — берём что уже рядом в первую очередь
            if d_to < 2.0:
                s *= 80.0
        ...
        # Гистерезис: не переключаемся если текущая цель рядом и новая не сильно лучше.
    ...
    try:
        ...
        while True:
            ...
            while "\n" in buffer:
                ...
                # Носители груза первыми — резервируют assignments раньше
